...
#CREDITS GO TO NEURALNINE FOR HIS TUTORIAL
#https://www.youtube.com/watch?v=a040VmmO-AY
...
class ChatbotAssistant:
    ...
    def bag_of_words(self, words):
        word_set = set(words)
        return np.array([1 if word in word_set else 0 for word in self.vocabulary])
    ...
    def process_message(self, input_message):
        ...
        bow = torch.from_numpy(np.array([self.bag_of_words(words)], dtype=np.float32))

        self.model.eval()
        with torch.no_grad():
            predictions = self.model(bow)
